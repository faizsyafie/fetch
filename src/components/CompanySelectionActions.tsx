...
// (Collapse). Blank state renders nothing so the two top bars match. Clear
// and Collapse each slide in/out independently too (not just the box as a
// whole), so gaining or losing just one of the two animates as well.
  ...
  return (
    ...
    >
      <div className="overflow-hidden">
        <div
          className={`flex shrink-0 items-center whitespace-nowrap rounded-lg border-l-2 py-1 pl-3 pr-1.5 ${accentPreset.border} ${accentPreset.softBg}`}
        >
          <div
            className="grid transition-[grid-template-columns] duration-200 ease-out"
            style={{ gridTemplateColumns: selectedCount > 0 ? "1fr" : "0fr" }}
          >
            <div className="overflow-hidden">
              <button
                type="button"
          ...
          <div
            className="grid transition-[grid-template-columns] duration-200 ease-out"
            style={{ gridTemplateColumns: expandedCount > 0 ? "1fr" : "0fr" }}
          >
            <div className="overflow-hidden">
              <button
                type="button"
